...
The upgrade operator writes candidate bytes, runs `cargo fmt`, reseals the content-addressed manifest, then invokes the complete ARIA verification lattice. Promotion occurs only after Rust compilation, all tests, release build and HTTP smoke closure pass. On failure the operator writes a failure receipt and preserves the prior safe state.

The authoritative result is produced on the target Windows machine.
